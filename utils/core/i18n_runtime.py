...
from utils.core.i18n import (
    has_portuguese_correction,
    has_text_translation,
    normalize_language,
    translate_text,
)
from utils.core.readability import improve_readability
...
def _looks_translatable(value: Any, widget: Any = None, prop: str = "") -> bool:
    text = str(value or "").strip()
    if not text or not any(char.isalpha() for char in text):
        return False
    if has_text_translation(text):
        return True
    if prop == "text" and hasattr(widget, "hint_text"):
        return False
    return has_portuguese_correction(text)
...
def _translate_changed_property(widget: Any, prop: str, value: Any) -> None:
    if getattr(widget, f"_i18n_setting_{prop}", False):
        return
    if not _looks_translatable(value, widget, prop):
        return
...
def localize_widget_tree(widget: Any, language: Any) -> None:
    ...
    while stack:
        ...
        for prop in TRANSLATABLE_PROPERTIES:
            ...
            if original is None:
                if not _looks_translatable(value, current, prop):
                    continue
                original = str(value)
                try:
