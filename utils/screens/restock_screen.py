...
class RestockScreen(MDScreen):
    ...
    def open_restock_for_product(self, product_id=None, query=None):
        self.set_mode("IN")
        lookup = {
        ...
        def focus_after_load(_rows=None):
            Clock.schedule_once(lambda dt: self._focus_restock_product(**lookup), 0)

        if self.products:
            focus_after_load(self.products)
            return
        self._product_load_callbacks.append(focus_after_load)
        self.load_products()

    def _find_product_for_focus(self, product_id=None, query=None):
        target_id = str(product_id or "").strip()
        query_text = str(query or "").strip().lower()

        if target_id:
            for product in self.products or []:
                pid, _name, _stock, _price, _cost, _barcode, _is_weight, _exp, _status, _avg, _days, _last = (
                    self._unpack_product(product)
                )
                if str(pid or "").strip() == target_id:
                    return product
        ...
        if query_text:
            for product in self.products or []:
                ...
                if any(query_text in term for term in terms):
                    return product
        return None

    def _focus_restock_product(self, product_id=None, query=None):
        product = self._find_product_for_focus(product_id=product_id, query=query)
        if product is not None:
            self.open_register_for_product(product)
            return True

        search_text = str(query or product_id or "").strip()
        if search_text and "search_input" in self.ids:
            self._pending_search = search_text
            self.ids.search_input.text = search_text
            self._apply_search_now()
        self.show_feedback("Produto nao encontrado automaticamente. Use a pesquisa para localizar.", "warning")
        return False
    ...
    def _reset_stock_register_form_state(self, blank_inputs=False, refresh_list=False, focus_search=False):
        if not self.ids:
            return
        self.selected_product = None
        ...
        self.ids.qty_input.text = "" if blank_inputs else "1"
        ...
        self.ids.update_day_field.text = "--" if blank_inputs else datetime.now().strftime("%d/%m/%Y")
        if refresh_list:
            self._apply_search_now()
        self._sync_stock_form_modal_from_hidden()
        if focus_search:
    ...
    def _product_row_from_alert_item(self, item=None, product_id=None, query=None):
        item = item or {}
        resolved_id = product_id if product_id not in (None, "") else (
        ...
        if resolved_id in (None, "") and not resolved_name:
            return None

        unit_text = str(item.get("unit") or "").strip().lower()
        is_weight = bool(item.get("is_weight")) or unit_text == "kg"
        return (
            resolved_id,
            resolved_name,
            item.get("stock") if item.get("stock") not in (None, "") else item.get("qty") or 0.0,
            item.get("price") or item.get("sale_price") or 0.0,
            item.get("cost") or item.get("unit_cost") or item.get("avg_cost") or 0.0,
            item.get("barcode") or item.get("code") or "",
            1 if is_weight else 0,
            item.get("expiry_date") or item.get("date"),
            item.get("status") or "ATIVO",
            item.get("avg_daily") or item.get("daily_avg") or 0.0,
            item.get("days_left"),
            item.get("last_update") or item.get("updated_at"),
        )

    def open_stock_register_modal(self, *args, blank=False, product_item=None, product_id=None, query=None):
        self._dismiss_stock_form_dialog()
        if blank:
            self._reset_stock_register_form_state(blank_inputs=True)
        elif product_item is not None or product_id not in (None, "") or query:
            self._reset_stock_register_form_state(blank_inputs=True)
            product_row = self._product_row_from_alert_item(
                item=product_item,
                product_id=product_id,
                query=query,
            )
            if product_row is not None:
                self.select_product(product_row)
    ...
    def on_search(self, text):
        ...
        self._search_ev = Clock.schedule_once(lambda dt: self._apply_search_now(), 0.3)
    ...
    def _show_products(self, rows):
        ...
        if self._products_render_index < len(self._products_render_rows):
            self._products_render_ev = Clock.schedule_interval(self._render_next_products_batch, 0.016)
    ...
    def clear_form(self):
        self._reset_stock_register_form_state(
            blank_inputs=False,
            refresh_list=True,
            focus_search=True,
        )
