...
from utils.config.device_config import get_device_settings, save_device_settings
from utils.config.env_loader import load_dotenv
from utils.core.focus_navigation import FormKeyboardController
from utils.core.i18n import language_label, language_options, normalize_language, translate
from utils.config.paths import APP_SETTINGS_FILE, ENV_FILE, data_path
from utils.config.system_status import collect_system_status
from utils.config.system_identity import DEFAULT_SYSTEM_NAME, get_system_name, normalize_system_name, save_system_name
from utils.hardware.thermal_printer import (
    get_default_printer_name,
    list_system_printers,
    print_thermal_receipt,
)
from kivy.lang import Builder
from utils.business.security_questions import QUESTIONS
from utils.business.email_service import save_smtp_settings
from utils.business.vat import VAT_RULES, DEFAULT_VAT_RULE_CODE, describe_vat_choice
...
class EmailVerificationDialog:
    """Confirma, sem expor o destinatario, o e-mail guardado na conta."""

    def __init__(self, username, on_verified=None):
    ...
    def show(self, send_code=True):
        if not self.dialog:
            ...
            content.add_widget(MDLabel(
                text="Introduza o codigo de 6 digitos enviado para o e-mail cadastrado.",
                theme_text_color="Secondary",
                size_hint_y=None,
                height=dp(46),
            ))
            self.code_field = MDTextField(
                hint_text="Codigo de verificacao",
                mode="rectangle",
                input_filter="int",
                max_text_length=6,
                size_hint_y=None,
            ...
            self.code_field.bind(on_text_validate=self.confirm)
            ...
            self.resend_button = MDFlatButton(text="REENVIAR", on_release=self.resend)
            self.confirm_button = MDRaisedButton(text="CONFIRMAR", on_release=self.confirm)
            self.dialog = MDDialog(
                ...
                auto_dismiss=False,
                buttons=[
                    MDFlatButton(text="FECHAR", on_release=self.dismiss),
                    self.resend_button,
                    self.confirm_button,
                ],
            )
        self.dialog.open()
        Clock.schedule_once(lambda *_: setattr(self.code_field, "focus", True), 0.1)
        if send_code:
            self.resend()

    def _set_busy(self, busy, message=""):
        if self.code_field:
            self.code_field.disabled = bool(busy)
        if self.confirm_button:
            self.confirm_button.disabled = bool(busy)
        if self.resend_button:
            self.resend_button.disabled = bool(busy)
        if self.status_label and message:
            self.status_label.text = message

    @staticmethod
    def _run_async(work, callback):
        def worker():
            try:
                payload, error = work(), None
            except Exception as exc:
                payload, error = None, str(exc)
            Clock.schedule_once(
                lambda _dt, result=payload, detail=error: callback(result, detail),
                0,
            )
    ...
    def resend(self, *args):
        if not self.username:
            return
        self._set_busy(True, "A enviar codigo...")

        def work():
            with get_db() as db:
                return db.request_email_verification(self.username)

        def done(result, error):
            self._set_busy(False)
            if error:
                self.status_label.text = "Nao foi possivel contactar o servico de e-mail."
                return
            result = result or {}
            if result.get("ok"):
                self.status_label.text = "Codigo enviado. Verifique a caixa de entrada e o spam."
                return
            reason = result.get("reason")
            if reason == "rate_limited":
                seconds = int(result.get("retry_after") or 60)
                self.status_label.text = f"Aguarde {seconds}s antes de reenviar."
            elif reason == "email_unavailable":
                self.status_label.text = "Envio de e-mail ainda nao configurado no servidor."
            else:
                self.status_label.text = "Nao foi possivel enviar o codigo."

        self._run_async(work, done)

    def confirm(self, *args):
        code = self.code_field.text.strip() if self.code_field else ""
        if len(code) != 6 or not code.isdigit():
            self.status_label.text = "Introduza os 6 digitos do codigo."
            return
        self._set_busy(True, "A confirmar codigo...")

        def work():
            with get_db() as db:
                return db.confirm_email_verification(self.username, code)

        def done(result, error):
            self._set_busy(False)
            if error:
                self.status_label.text = "Nao foi possivel confirmar o codigo."
                return
            result = result or {}
            if result.get("ok"):
                self.dismiss()
                if callable(self.on_verified):
                    self.on_verified()
                return
            reason = result.get("reason")
            if reason == "expired":
                self.status_label.text = "O codigo expirou. Solicite outro."
            elif reason == "too_many_attempts":
                self.status_label.text = "Tentativas excedidas. Solicite outro codigo."
            else:
                remaining = result.get("remaining")
                suffix = f" Restam {remaining} tentativas." if remaining is not None else ""
                self.status_label.text = f"Codigo invalido.{suffix}"
...
class SmtpSettingsDialog:
    """Configura a conta de envio da instalacao sem pedir suporte externo."""
    ...
    def _field_row(self, field, title, help_text):
        container = MDBoxLayout(orientation="vertical", spacing=dp(2), size_hint_y=None, height=dp(56))
        top_row = MDBoxLayout(orientation="horizontal", spacing=dp(4), size_hint_y=None, height=dp(56))
        top_row.add_widget(field)
        help_label = MDLabel(
            text=help_text,
            theme_text_color="Custom",
            text_color=[0.25, 0.32, 0.42, 1],
            font_style="Caption",
            size_hint_y=None,
            height=0,
            opacity=0,
        )
        help_label.bind(width=lambda inst, width: setattr(inst, "text_size", (max(0, width - dp(8)), None)))

        def toggle_help(_button):
            expanded = bool(help_label.opacity)
            if expanded:
                help_label.height = 0
                help_label.opacity = 0
                container.height = dp(56)
            else:
                help_label.height = dp(42)
                help_label.opacity = 1
                container.height = dp(100)

        def set_help(visible):
            if visible and not help_label.opacity:
                help_label.height = dp(42)
                help_label.opacity = 1
                container.height = dp(100)
            elif not visible and help_label.opacity:
                help_label.height = 0
                help_label.opacity = 0
                container.height = dp(56)
        ...
        top_row.add_widget(MDIconButton(
            ...
            theme_text_color="Custom",
            text_color=[0.10, 0.38, 0.75, 1],
            size_hint_x=None,
            width=dp(42),
            on_release=toggle_help,
        ))
        field.bind(focus=lambda _field, focused: set_help(bool(focused)))
        container.add_widget(top_row)
        container.add_widget(help_label)
        return container
    ...
    def show(self):
        if not self.dialog:
            ...
            help_banner = MDCard(
                ...
                radius=[dp(12)],
                elevation=0,
                md_bg_color=[0.91, 0.95, 1, 1],
            )
            help_banner.add_widget(MDIcon(
                icon="information-outline",
                theme_text_color="Custom",
                text_color=[0.10, 0.38, 0.75, 1],
                size_hint_x=None,
            ...
            help_text = MDBoxLayout(orientation="vertical", spacing=dp(2))
            help_text.add_widget(MDLabel(
                text="Gmail: ative a verificacao em duas etapas e crie uma senha de aplicacao.",
                theme_text_color="Custom", text_color=[0.12, 0.18, 0.28, 1],
                font_style="Caption", size_hint_y=None, height=dp(34),
            ))
            help_text.add_widget(MDFlatButton(
                text="ABRIR SENHAS DO GOOGLE",
                theme_text_color="Custom", text_color=[0.10, 0.38, 0.75, 1],
                size_hint_x=None, width=dp(220), size_hint_y=None, height=dp(34),
                on_release=lambda *_: webbrowser.open("https://myaccount.google.com/apppasswords"),
            ))
            help_banner.add_widget(help_text)
            content.add_widget(help_banner)
            self.host = MDTextField(
                hint_text="Servidor SMTP",
                text="smtp.gmail.com",
                mode="rectangle",
                size_hint_y=None,
                height=dp(56),
            ...
            self.username = MDTextField(
                hint_text="E-mail remetente",
                mode="rectangle",
                size_hint_y=None,
                height=dp(56),
            )
            self.password = MDTextField(
                hint_text="Senha de aplicacao",
                password=True,
                mode="rectangle",
                size_hint_y=None,
                height=dp(56),
            )
            self.from_email = MDTextField(
                hint_text="E-mail remetente (confirmacao)",
                mode="rectangle",
                size_hint_y=None,
                height=dp(56),
            )
            self.status = MDLabel(
                text="Depois de guardar, teste a recuperacao de senha para confirmar o envio.",
                theme_text_color="Secondary",
                size_hint_y=None,
                height=dp(36),
            )
            content.add_widget(self._field_row(
                self.host, "Servidor SMTP",
                "Para Gmail use smtp.gmail.com. Este servidor entrega as mensagens de recuperacao.",
            ))
            content.add_widget(self._field_row(
                self.username, "E-mail remetente",
                "Informe o endereco da conta que vai autenticar no servidor SMTP.",
            ))
            content.add_widget(self._field_row(
                self.password, "Palavra-passe de aplicacao",
                "No Gmail, crie uma senha de aplicacao em myaccount.google.com/apppasswords. "
                "Cole aqui o codigo de 16 caracteres, nao a senha normal.",
            ))
            content.add_widget(self._field_row(
                self.from_email, "E-mail remetente (confirmacao)",
                "Repita o mesmo e-mail remetente. Ele aparecera como origem das mensagens.",
            ))
            content.add_widget(self.status)
            self.dialog = MDDialog(
                title="Configurar envio de e-mail",
                type="custom",
                content_cls=content,
                auto_dismiss=False,
                buttons=[
                    MDFlatButton(text="CANCELAR", on_release=self.dismiss),
                    MDRaisedButton(text="GUARDAR", on_release=self.save),
                ],
            )
        self.dialog.open()

    def save(self, *args):
        username = self.username.text.strip() if self.username else ""
        password = self.password.text.strip() if self.password else ""
        from_email = self.from_email.text.strip() if self.from_email else username
        try:
            result = save_smtp_settings(
                username=username,
                password=password,
                from_email=from_email,
                host=self.host.text.strip() if self.host else "smtp.gmail.com",
                port=587,
                from_name="SIGE MPE",
                use_tls=True,
                use_ssl=False,
        ...
        MDDialog(
            title="Sucesso",
            text=f"Envio configurado para {result['from_email']}",
            buttons=[MDFlatButton(text="OK")],
        ).open()
...
class RecoveryCodesDialog:
    ...
    def show(self):
        ...
        username = str(getattr(app, "current_user", "") or "").strip() if app else ""
        if not username:
            MDDialog(title="Indisponivel", text="Nao foi possivel identificar o administrador.", buttons=[MDFlatButton(text="OK")]).open()
            return
        result = self.db.generate_recovery_codes(username, count=8)
        if not result.get("ok"):
            MDDialog(title="Erro", text="Nao foi possivel gerar os codigos.", buttons=[MDFlatButton(text="OK")]).open()
            return
        ...
        self.dialog = MDDialog(
            title="Codigos de emergencia",
            type="custom", content_cls=content, auto_dismiss=False,
            buttons=[MDFlatButton(text="FECHAR", on_release=self.dismiss)],
        )
        self.dialog.open()
...
class ChangeAdminDataDialog:
    ...
    def __init__(self):
        ...
        self._email_verification_dialog = None
    ...
    def show(self):
        ...
        if not self.dialog:
            ...
            self.new_email = MDTextField(
                hint_text='Novo e-mail de recuperacao (opcional)',
                mode='rectangle',
                size_hint_y=None,
                height=dp(56)
            )
            content.add_widget(self.new_email)
    ...
    def save_changes(self, *args):
        ...
        if new_email and not _looks_like_email(new_email):
            self.show_message('Erro', 'Informe um endereco de e-mail valido')
            return
        ...
        try:
            with get_db() as db:
                ...
                if new_email and not db.update_user_email(updated_username, new_email):
                    self.show_message('Erro', 'O e-mail e invalido ou ja esta associado a outra conta')
                    return
...
class AddUserDialog:
    ...
    def show(self):
        if not self.dialog:
            ...
            self.email = MDTextField(
                hint_text='E-mail de recuperacao',
                mode='rectangle',
                size_hint_y=None,
                height=dp(56)
            )
    ...
    def save_user(self, *args):
        ...
        if not username or not email or not password or not self.selected_role:
        ...
        if len(password) < 8:
            self.show_message('Erro', 'A senha deve ter no minimo 8 caracteres')
            return
...
class SystemLogsDialog:
    ...
    def _action_style(self, action):
        styles = {
            ...
            "VERIFY_EMAIL": ("email-check-outline", "success"),
            "RESET_PASSWORD_EMAIL": ("email-lock-outline", "warning"),
    ...
    def _cancel_pending_render(self):
        event = getattr(self, "_render_event", None)
    ...
    def _compact_log_details(self, details, max_chars=260):
        text = " ".join(str(details or "").split())
        if len(text) > max_chars:
            return text[: max_chars - 3].rstrip() + "..."
        return text

    def _build_log_card(self, log, tokens):
        ...
        info_box = MDBoxLayout(orientation='vertical', spacing=dp(5), adaptive_height=True, size_hint_y=None)
        info_box.bind(minimum_height=info_box.setter("height"))

        header = MDBoxLayout(orientation='horizontal', size_hint_y=None, height=dp(25))
        user_label = self._make_label(
            text=f'[b]{username or "Sistema"}[/b] ({role or "sem perfil"})',
            markup=True,
            color=tokens.get("text_primary", [0.15, 0.20, 0.30, 1]),
            font_style='Body2'
        )
        ...
        detail_text = self._compact_log_details(details)
        if detail_text:
    ...
    def _render_logs_incrementally(self, logs, tokens):
        self._cancel_pending_render()
        logs = list(logs or [])
        total = len(logs)

        def add_batch(start_index):
            end_index = min(start_index + self._render_batch_size, total)
            for log in logs[start_index:end_index]:
                self.logs_list.add_widget(self._build_log_card(log, tokens))

            if end_index < total:
                self._render_event = Clock.schedule_once(
                    lambda _dt, next_index=end_index: add_batch(next_index),
                    0,
                )
                return
    ...
    def _fetch_logs(self, limit=100):
        ...
        with get_db() as db:
            try:
                return db.get_user_logs(
                    user_filter,
                    action_filter,
                    role_filter,
                    limit=limit,
                    exclude_actions=excluded_actions,
                    details_limit=260 if limit else None,
                ) or []
            except TypeError:
                rows = db.get_user_logs(user_filter, action_filter, role_filter, limit=limit)
    ...
    def load_logs(self):
        ...
        def worker():
            ...
            error = None
            try:
                logs = self._fetch_logs(limit=self._visible_limit)
            except Exception as exc:
                error = exc
        ...
        def apply_result(logs, error):
            ...
            self._render_logs_incrementally(logs, tokens)
...
class EnhancedSystemLogsDialog:
    ...
    def _action_style(self, action):
        ...
        self._action_styles = {
            ...
            "VERIFY_EMAIL": {"icon": "email-check-outline", "tone": "success"},
            "RESET_PASSWORD_EMAIL": {"icon": "email-lock-outline", "tone": "warning"},
...
class SystemStatusDialog:
    # Dialogo de leitura rapida para saude operacional do sistema.
    def __init__(self, db=None):
        self.db = db
        self.dialog = None
        self.content = None
    ...
    def show(self):
        ...
        self.dialog = MDDialog(
            ...
            buttons=[
                MDFlatButton(text='FECHAR', on_release=lambda _x: self.dialog.dismiss()),
                MDFlatButton(text='CRIAR BACKUP', on_release=lambda _x: self.create_backup()),
                MDRaisedButton(text='ATUALIZAR', on_release=lambda _x: self.refresh()),
            ],
        )
        self.dialog.open()
        Clock.schedule_once(lambda _dt: self.refresh(), 0.1)
    ...
    def refresh(self):
        ...
        def worker():
            try:
                status = collect_system_status(self.db, include_api_health=True)
                Clock.schedule_once(lambda _dt: self._render_status(status), 0)
            except Exception as exc:
                Clock.schedule_once(lambda _dt: self._render_error(str(exc)), 0)

        Thread(target=worker, daemon=True).start()

    def create_backup(self):
        if not self.db or not hasattr(self.db, 'create_verified_backup'):
            self._render_error('Backup manual indisponivel para a ligacao atual.')
            return

        self.content.clear_widgets()
        self.content.add_widget(MDLabel(
            text='A criar backup verificado da base de dados...',
            theme_text_color='Secondary',
            size_hint_y=None,
            height=dp(32),
        ))
        progress = MDProgressBar(size_hint_y=None, height=dp(4))
        progress.value = 70
        self.content.add_widget(progress)

        def worker():
            try:
                result = self.db.create_verified_backup()
                Clock.schedule_once(lambda _dt: self._render_backup_result(result), 0)
            except Exception as exc:
                Clock.schedule_once(lambda _dt: self._render_error(str(exc)), 0)

        Thread(target=worker, daemon=True).start()

    def _render_backup_result(self, result):
        ok = bool((result or {}).get('ok'))
        path = str((result or {}).get('path') or '')
        reason = str((result or {}).get('reason') or '')
        if ok:
            message = f'Backup criado e verificado com sucesso.\n{path}'
        else:
            message = f'Nao foi possivel criar backup.\n{reason or "Erro desconhecido"}'

        self.content.clear_widgets()
        self.content.add_widget(MDLabel(
            text=message,
            theme_text_color='Primary' if ok else 'Error',
            size_hint_y=None,
            height=dp(72),
            text_size=(dp(520), None),
        ))
        Clock.schedule_once(lambda _dt: self.refresh(), 1.2 if ok else 2.2)
    ...
    def _render_loading(self):
        ...
        self.content.clear_widgets()
        self.content.add_widget(MDLabel(
            text='A verificar base de dados, API, backups e logs...',
            theme_text_color='Secondary',
            size_hint_y=None,
            height=dp(32),
        ))
    ...
    def _render_error(self, message):
        ...
        self.content.clear_widgets()
        self.content.add_widget(MDLabel(
            text=f'Nao foi possivel recolher o estado do sistema.\n{message}',
            theme_text_color='Error',
            size_hint_y=None,
            height=dp(72),
        ))
    ...
    def _render_status(self, status):
        ...
        latest_backup = files.get('latest_backup') or {}
        latest_log = files.get('latest_log') or {}
        ...
        if alerts:
            ...
            for alert in alerts[:4]:
                self.content.add_widget(self._alert_row(alert))

        rows = [
            ('Ambiente', status.get('app_env') or '-'),
            ('Modo da base', status.get('db_mode') or '-'),
            ('Base de dados', db_info.get('path') or '-'),
            ('Tamanho da base', self._format_bytes(db_info.get('size_bytes'))),
            ('Integridade SQLite', self._ok_text((db_info.get('integrity') or {}).get('ok'))),
            ('API', api_info.get('base_url') or '-'),
            ('Saude da API', self._health_text(api_health)),
            ('Chave da API', 'Configurada' if api_info.get('has_api_key') else 'Ausente'),
            ('Ultimo backup', automation.get('last_backup_run') or 'Sem registo'),
            ('Estado do backup', automation.get('last_backup_status') or 'Sem registo'),
            ('Ultimo ficheiro backup', latest_backup.get('path') or 'Nenhum encontrado'),
            ('Ultimo log', latest_log.get('path') or 'Nenhum encontrado'),
        ]

        for label, value in rows:
            self.content.add_widget(self._row(label, value))

        self.content.add_widget(MDLabel(
            text=f"Atualizado em {status.get('generated_at') or '-'}",
            font_style='Caption',
            theme_text_color='Secondary',
            size_hint_y=None,
            height=dp(24),
    ...
    def _alert_row(self, alert):
        level = str((alert or {}).get('level') or '').lower()
        message = str((alert or {}).get('message') or '-')
        color = 'Error' if level == 'critical' else ('Secondary' if level == 'ok' else 'Primary')
        prefix = 'OK' if level == 'ok' else ('CRITICO' if level == 'critical' else 'AVISO')
        box = MDBoxLayout(
            orientation='vertical',
            spacing=dp(2),
            size_hint_y=None,
    ...
    def _health_text(health):
        ...
        if ok is True:
            return 'Online'
        if ok is False:
            return f'Indisponivel ({reason})' if reason else 'Indisponivel'
...
class AdminSettingsScreen(MDScreen):
    ...
    def __init__(self, app, **kwargs):
        ...
        self._smtp_settings_dialog = None
        self._recovery_codes_dialog = None
    ...
    def on_enter(self):
        self._bind_app_language()
        Clock.schedule_once(lambda dt: self._sync_device_settings_state(), 0)
        Clock.schedule_once(lambda dt: self._sync_language_state(), 0)
        Clock.schedule_once(lambda dt: self._sync_system_name_state(), 0)
        Clock.schedule_once(self._init_badge, 0.1)
        Clock.schedule_once(lambda dt: self._start_ai_polling(), 0.15)
        Clock.schedule_once(lambda dt: self._refresh_vat_overview(), 0.05)
        Clock.schedule_once(self._apply_responsive_layout, 0)
    ...
    def configure_email_sender(self):
        if not self._smtp_settings_dialog:
            self._smtp_settings_dialog = SmtpSettingsDialog()
        self._smtp_settings_dialog.show()

    def generate_recovery_codes(self):
        if not self._recovery_codes_dialog:
            self._recovery_codes_dialog = RecoveryCodesDialog(self.db)
        self._recovery_codes_dialog.show()
    ...
    def view_system_status(self):
        SystemStatusDialog(self.db).show()
