...
from utils.core.i18n import normalize_language, translate
...
def _current_language_code():
    ...
    try:
        from utils.config.app_config import get_app_settings
...
def _generate_ai_insights(payload):
    ...
    prompt = (
        ...
        "Analise os dados e devolva um JSON completo, com conclusões claras e úteis."
    )
...
def _build_sales_answer(snapshot, route):
    ...
    if route["mentions_promo"]:
        if snapshot["promo_count"] <= 0:
            return [f"Não houve vendas promocionais nos últimos {lookback} dias."]
        return [
...
def _build_stock_answer(snapshot, route):
    ...
    if route["ask_alert"] and low_stock:
        return [f"Há risco imediato de ruptura em {len(low_stock)} itens. O mais crítico é {low_stock[0][0]}."]

    return [
        f"Stock: {len(low_stock)} itens em nível baixo e {len(exp7)} produtos a vencer em 7 dias, com base nos últimos {lookback} dias.",
    ]
...
def _build_losses_answer(snapshot, route):
    ...
    return [f"No período, foram registadas {snapshot['loss_count']} perdas, num total de {_fmt_mzn(snapshot['loss_cost'])}."]
...
def _build_direct_lines(question, snapshot, route):
    ...
    lines = [
        ...
        f"Atenção a {low_count} itens com stock baixo e {exp7} itens a vencer nos próximos 7 dias.",
    ]
    return lines
...
def _build_alerts(snapshot):
    ...
    if snapshot["sales_count"] == 0:
        alerts.append("Não houve vendas no período analisado.")
...
def _build_api_overlay(question, snapshot, local_answer, is_business_question=True, route=None):
    ...
    prompt = (
        ...
        "Sem Markdown nem código. Use um tom natural e direto, com até 2 conclusões e 1 ação.\n\n"
