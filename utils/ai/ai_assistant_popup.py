...
from utils.ai.ai_insights import answer_management_question, can_use_ai_api, build_admin_insights
...
class ManagementAIAssistantPopup(Popup):
    ...
    def __init__(self, db=None, ai_name="NomeDaIA", scope="management", **kwargs):
        super().__init__(**kwargs)
        self.db = db
        self.ai_name = ai_name or "NomeDaIA"
        self.scope = str(scope or "management").strip().lower()
        self._busy = False
        ...
        self._default_quick_questions = [
            ...
            "Que ação devo priorizar hoje para reduzir perdas e evitar rupturas?",
        ]
        self._setup_popup()
    ...
    def _on_ask(self, *_):
        ...
        if not question:
            ...
            return
        if not self._question_is_in_scope(question):
            self._render_error(
                "ESCOPO DO GERENTE: posso ajudar com produtos, vendas, stock, "
                "perdas, caixa, pagamentos, validade, reposicao e operacao da loja."
            )
            return
    ...
    def _question_is_in_scope(self, question):
        if self.scope != "manager":
            return True
        text = str(question or "").strip().lower()
        allowed_terms = (
            "produto", "produtos", "stock", "estoque", "venda", "vendas",
            "receita", "faturacao", "facturacao", "lucro", "margem", "preco",
            "promocao", "validade", "vence", "vencem", "perda", "perdas",
            "caixa", "pagamento", "pagamentos", "recibo", "operador",
            "operacao", "loja", "ruptura", "reposicao", "repor", "categoria",
            "turno", "atividade", "alerta", "priorizar", "cliente", "clientes",
        )
    ...
    def _extract_primary_answer_lines(self, result):
        ...
        return [summary] if summary else ["Não há resposta disponível."]
    ...
    def _build_dynamic_quick_questions(self):
        ...
        if low_stock or exp7 or exp15:
            questions.append("Qual é o alerta mais crítico de hoje e o que devo fazer primeiro?")
...
# ─── ENTRY POINT ─────────────────────────────────────────────────────────────
def open_management_ai_assistant(db=None, ai_name=None, scope="management"):
    app = App.get_running_app()
    resolved_name = ai_name or getattr(app, "assistant_name", "JoeIA")
    popup = ManagementAIAssistantPopup(db=db, ai_name=resolved_name, scope=scope)
    popup.open()
    return popup
