...
function Find-ReleaseRoot {
    ...
    while ($true) {
        if ((Test-Path (Join-Path $current "SIGEMPEAdmin.exe")) -and (Test-Path (Join-Path $current "config\app.json"))) {
            return $current
        }
...
$backupRoot = Join-Path $env:TEMP ("SIGEMPEAdminUpdate_" + (Get-Date -Format "yyyyMMddHHmmss"))
...
$exePath = Join-Path $installRoot "SIGEMPEAdmin.exe"
...
Write-Host "Admin atualizado em: $installRoot"
