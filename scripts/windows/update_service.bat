...
for /f "usebackq delims=" %%I in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$cfg = Get-Content 'config/service.json' -Raw | ConvertFrom-Json; $root = (Get-Location).Path; $nssm = $cfg.nssm_path; if (-not [System.IO.Path]::IsPathRooted($nssm)) { $nssm = Join-Path $root $nssm }; $python = $cfg.python_executable; if ($python -and $python -ne 'python' -and -not [System.IO.Path]::IsPathRooted($python)) { $python = Join-Path $root $python }; $entry = $cfg.entrypoint; if ($entry -and -not $entry.StartsWith('-') -and -not [System.IO.Path]::IsPathRooted($entry)) { $entry = Join-Path $root $entry }; $work = $cfg.working_directory; if (-not [System.IO.Path]::IsPathRooted($work)) { $work = Join-Path $root $work }; $stdout = $cfg.stdout_log; if (-not [System.IO.Path]::IsPathRooted($stdout)) { $stdout = Join-Path $root $stdout }; $stderr = $cfg.stderr_log; if (-not [System.IO.Path]::IsPathRooted($stderr)) { $stderr = Join-Path $root $stderr }; @($cfg.name, $nssm, $python, $entry, $work, $stdout, $stderr) | ForEach-Object { $_ }"`) do (
