...
if (-not $SkipBuild) {
    python -m PyInstaller --noconfirm manager_app.spec
}
...
foreach ($folder in @("user", "utils", "manager")) {
    $sourceFolder = Join-Path $internal $folder
...
Copy-Item -Force (Join-Path $root "scripts\install_manager_client.ps1") (Join-Path $installDir "install_manager_client.ps1")
Copy-Item -Force (Join-Path $root "scripts\update_manager_client.ps1") (Join-Path $installDir "update_manager_client.ps1")
...
Copy-Item -Force (Join-Path $root "docs\INSTALACAO_MANAGER.md") (Join-Path $installDir "INSTALACAO_MANAGER.md")
...
Guia completo:
  install\INSTALACAO_MANAGER.md
"@

$readme | Set-Content -Encoding UTF8 (Join-Path $dist "LEIA-ME-MANAGER.txt")

Write-Host "Build pronto em: $dist"
Write-Host "Copie a pasta SIGEMPEManager inteira para os outros computadores."
